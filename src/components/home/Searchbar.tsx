...
export default function Searchbar() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    setQuery((prev) => ({ ...prev, minPrice: pricelimit[query.type].min.toFixed(0), maxPrice: pricelimit[query.type].max.toFixed(0), [e.target.name]: e.target.value }));
    console.log(Number(query.minPrice), Number(query.maxPrice));
  };

  const switchType = (val: string) => {
    setQuery((prev) => ({ ...prev, type: val as ('buy' | 'rent'), minPrice: pricelimit[val as ('buy' | 'rent')].min.toFixed(0), maxPrice: pricelimit[val as ('buy' | 'rent')].max.toFixed(0) }));
    if (formRef.current)
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const data: IFormInput = {
      type: query.type,
      minPrice: (query.minPrice as string) === '' ? pricelimit[query.type].min.toFixed(0) : query.minPrice,
      maxPrice: (query.maxPrice as string) === '' ? pricelimit[query.type].max.toFixed(0) : query.maxPrice,
    };
    ...
    console.log(Number(data.minPrice), Number(data.maxPrice));
    ...
    navigate(`/list?city=${city.city}&type=${data.type}&minPrice=${data.minPrice}&maxPrice=${data.maxPrice}&lat=${city ? city.coordinates.lat : ""}&lon=${city ? city.coordinates.lon : ""}`);
  ...
  return (
    <div className="searchbar font-poppins font-medium">
      ...
      <form
        ...
        action="POST"
        className="flex h-auto flex-col justify-between gap-3 overflow-hidden rounded-md border-zinc-500 md:flex-row md:border md:h-16"
      >
        ...
        <div className="h-full relative">
          <input
            ...
            className=" w-full h-full remove-arrow rounded-md border bg-transparent px-3 py-3 outline-none focus:border-blue-600 md:w-[140px] md:border-none xl:w-[180px]"
            type="number"
            name="minPrice"
            id="minPrice"
            min={1000}
          ...
          />
          <span className="absolute bottom-[1px] right-[1px] flex h-5 items-center rounded-l-[5px] border-none text-blue-300  font-medium px-2 shadow-xl">
        ...
        <div className="h-full relative">
          <input
            onChange={handleChange}
            className=" w-full h-full relative remove-arrow rounded-md border bg-transparent px-3 py-3 outline-none focus:border-blue-600 md:w-[140px] md:border-none xl:w-[180px]"
            type="number"
            name="maxPrice"
            id="maxPrice"
            min={1000}
