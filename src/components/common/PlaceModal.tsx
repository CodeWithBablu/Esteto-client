...
import { Address, City, Country, GeoJSONFeature, toastMessage, useCityStore } from "@/lib";
...
export default function PlaceModal<T extends Country | City | Address>({ type, data, setData }: Props<T>) {
  ...
  const handleClick = (data: T | GeoJSONFeature) => {
    if (type === 'place') {
      const updatedData = {
        ...
        city: (data as GeoJSONFeature).properties.context.district.name.toLowerCase(),
  ...
  return (
    <div className={clsx(
      {
        'h-full': type !== 'place',
        'absolute left-0 right-0 m-auto sm:right-auto sm:left-5 top-5 z-10 h-14 w-[95%] sm:w-80': type === "place",
      }
    )}>
        ...
        <Dialog.Trigger>
          <div className="h-full">
            ...
            <input
              id="place"
              readOnly
              value={data ? (type === 'place' ? (data as Address).place : (data as City).city) : (type === 'place' ? 'Search location' : 'City Location')}
              name="place"
              type="text"
              className={clsx(
                'placeholder-gray-400 w-full select-none',
                {
                  'h-14 text-lg rounded-lg bg-zinc-950/80 backdrop-blur-sm text-gray-100 px-6 outline-none': type === "place",
                  'w-full h-full rounded-md border bg-transparent px-3 py-3 outline-none focus:border-blue-600 md:w-[200px] md:border-none': type === "city",
                  'text-gray-400': !data,
                  'text-zinc-800': data && type !== 'place',
                  'text-gray-100': data && type === 'place',
                })}
              placeholder={type === 'place' ? 'Search location' : 'City Location'}
              autoComplete="off"
            />
          </div>
        </Dialog.Trigger>
            ...
            {response.length > 0 && (
              <>
                {response.map((item: T | GeoJSONFeature, index) => (
                  <div
                    className="cursor-pointer rounded-md pl-6 py-1 text-gray-100 hover:bg-blue-500"
                    key={index}
                    onClick={() => handleClick(item)}
                  >
                    {type === "place" && (
                      <div className="flex flex-col gap-1 border-b-[1px] pb-1 border-gray-600 w-full">
                        <h2 className="font-medium">{(item as GeoJSONFeature).properties.name}</h2>
                        <h3 className="text-gray-200">{(item as GeoJSONFeature).properties.full_address}</h3>
                      </div>
                    )}
                    {
                      (
